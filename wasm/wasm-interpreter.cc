...
//ADD HERE renamed addr to addr1 as it is used previously
    byte* addr1 = wasm_context_->mem_start + operand.offset + index;
    WasmValue result(converter<ctype, mtype>{}(ReadLittleEndianValue<mtype>(addr1)));
    if (FLAG_wasm_taint && FLAG_taint_heap) {
      if (shadow_heap.count(addr1)) {
        result.setTaint(shadow_heap[addr1]);
      }
    }
//END ADD
...
#define EXECUTE_UNOP(name, ctype, exec_fn)                  \
  case kExpr##name: {                                       \
    TrapReason trap = kTrapCount;                           \
    WasmValue val = Pop();                                  \
    auto result = exec_fn(val.to<ctype>(), &trap);          \
    possible_nondeterminism_ |= has_nondeterminism(result); \
    if (trap != kTrapCount) return DoTrap(trap, pc);        \
    WasmValue res = WasmValue(result);                      \
